...
type ShapeType = 'rectangle' | 'circle' | 'polygon' | 'triangle' | 'line' | 'arrow' | 'star' | 'image';
...
export const ModernToolbar: React.FC<ModernToolbarProps> = ({
  ...
  const getCurrentCursorTool = () => cursorTools.find(tool => tool.id === cursorMode);
  const getCurrentShapeTool = () => shapeTools.find(tool => tool.id === shapeMode);
  const getCurrentDrawingTool = () => drawingTools.find(tool => tool.id === drawingMode);
      ...
      <div className="modern-toolbar">
        <div className="toolbar-left">
          ...
          <div className="tool-group">
            <button
              ref={cursorButtonRef}
              className={`main-tool-button ${cursorMode !== 'select' || selectedTool === null ? 'active' : ''}`}
              onClick={() => setShowCursorDropdown(!showCursorDropdown)}
              title={getCurrentCursorTool()?.name}
            >
              {getCurrentCursorTool()?.icon}
              <svg className="dropdown-arrow" width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            {showCursorDropdown && (
              <div className="tool-dropdown">
                {cursorTools.map((tool) => (
                  <button
                    key={tool.id}
                    className={`dropdown-item ${cursorMode === tool.id ? 'active' : ''}`}
          ...
          <div className="tool-group">
            <button
              ref={shapeButtonRef}
              className={`main-tool-button ${(selectedTool === 'rectangle' || selectedTool === 'circle') ? 'active' : ''}`}
            ...
            {showShapeDropdown && (
              <div className="tool-dropdown">
                {shapeTools.map((tool) => (
                  <button
                    key={tool.id}
                    className={`dropdown-item ${shapeMode === tool.id ? 'active' : ''}`}
          ...
          <div className="tool-group">
            <button
              ...
              className={`main-tool-button ${showDrawingToolbar ? 'active' : ''}`}
              onClick={() => setShowDrawingDropdown(!showDrawingDropdown)}
            ...
            {showDrawingDropdown && (
              <div className="tool-dropdown">
                {drawingTools.map((tool) => (
                  <button
                    key={tool.id}
                    className={`dropdown-item ${drawingMode === tool.id ? 'active' : ''}`}
